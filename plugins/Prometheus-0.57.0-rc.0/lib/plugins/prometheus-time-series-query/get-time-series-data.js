...
export const getTimeSeriesData = async (spec, context, abortSignal)=>{
    ...
    const selectedDatasource = datasourceSelectValueToSelector(spec.datasource ?? DEFAULT_PROM, context.variableState, listDatasourceSelectItems) ?? DEFAULT_PROM;
    const datasource = await context.datasourceStore.getDatasource(selectedDatasource);
    const datasourceScrapeInterval = Math.trunc(milliseconds(parseDurationString(datasource.plugin.spec.scrapeInterval ?? DEFAULT_SCRAPE_INTERVAL)) / 1000);
    ...
    const minStep = getDurationStringSeconds(// resolve any variable that may have been provided
    // TODO add a validation check to make sure the variable is a DurationString, to avoid the back & forth cast here
    replaceVariables(spec.minStep, context.variableState)) ?? datasourceScrapeInterval;
    ...
    return data.result.map((res)=>{
        ...
        if (histograms) {
            return {
                ...
                formattedName,
                labels: metric,
                values: histograms.map((histogram)=>parseValueTuple([
                        histogram[0],
                        histogram[1].sum
                    ])),
