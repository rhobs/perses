...
import (
	...
	"github.com/rhobs/perses/internal/api/validate"
...
// checkSecretPermission ensures that the user that creates/updates a global datasource with a secret actually has the
// secret reader permission.
func (s *service) checkSecretPermission(ctx echo.Context, datasource *v1.GlobalDatasource) error {
	...
	if proxyErr != nil {
		logrus.WithError(proxyErr).WithField("datasource", datasource.Metadata.Name).Error("unable to build or find the config in the datasource spec")
	...
	if ok := s.authz.HasPermission(ctx, role.ReadAction, v1.WildcardProject, role.GlobalSecretScope); !ok {
		return apiInterface.HandleForbiddenError(fmt.Sprintf("missing '%s' global permission for '%s' kind", role.ReadAction, role.GlobalSecretScope))
	}
