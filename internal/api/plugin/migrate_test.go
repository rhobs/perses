...
func TestMig_Migrate(t *testing.T) {
	...
	for _, test := range testSuite {
		t.Run(test.title, func(t *testing.T) {
			inputGrafanaDashboardRaw := testUtils.ReadFile(filepath.Join("migrate", testDataFolder, test.inputGrafanaDashboardFile))
			expectedPersesDashboardRaw := testUtils.ReadFile(filepath.Join("migrate", testDataFolder, test.expectedPersesDashboardFile))
			...
			dashboard, err := pl.Migration().Migrate(grafanaDashboard)
			if err != nil {
				t.Fatal(err)
			}
